...
| `deterministic.planner_runtime_node` | Planner runtime checkpoint seam | `STATUS:BUILT` | `graph.py` registers `planner_runtime` and records checkpoint metadata and follow-up state. |
...
The status table is intentionally machine-checkable. It keeps this plan aligned with the README's description of the portal as intentionally small and server-rendered.
