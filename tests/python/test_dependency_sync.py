...
class TestLockFiles:
    ...
    def test_lock_file_header(self) -> None:
        """Lock files should have pip-compile header."""
        for lock_name in ["requirements.lock", "requirements-dev.lock"]:
            lock_file = REPO_ROOT / lock_name
            if lock_file.exists():
                content = lock_file.read_text(encoding="utf-8")
                # Accept both old and new pip-compile header formats
                has_header = (
                    "@generated by pip-compile" in content
                    or "autogenerated by pip-compile" in content
                )
                assert has_header, (
                    f"{lock_name} is missing pip-compile header. "
                    f"Regenerate with pip-compile."
                )
