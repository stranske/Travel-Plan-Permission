...
def test_mapping_fields_match_canonical_contract():
    ...
    assert mapping.missing_fields(CANONICAL_TRIP_FIELDS) == []
    assert set(mapping.cells).issuperset(
        {
            "traveler_name",
            "business_purpose",
            "destination_zip",
...
def test_version_mismatch_requires_opt_in(tmp_path: Path):
    ...
    data["templates"][DEFAULT_TEMPLATE_VERSION]["metadata"]["template_id"] = "ITIN-2025.2"
