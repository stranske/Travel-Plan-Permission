...
def test_cli_success_creates_spreadsheet(
...
def test_cli_validation_error_returns_error(
    tmp_path: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    input_path = tmp_path / "plan.json"
    output_path = tmp_path / "output.xlsx"
...
def test_cli_help_shows_usage(capsys: pytest.CaptureFixture[str]) -> None:
