...
def test_complexity_cli_rejects_a_changed_function_over_the_ceiling(
    ...
    source_root = tmp_path / "src" / "travel_plan_permission"
    ...
    for command in (
        ...
        ["git", "add", "src/travel_plan_permission/sample.py"],
    ...
    subprocess.run(["git", "add", "src/travel_plan_permission/sample.py"], cwd=tmp_path, check=True)
    subprocess.run(["git", "commit", "--quiet", "-m", "deliberate break"], cwd=tmp_path, check=True)

    result = subprocess.run(
        [
            sys.executable,
            str(Path(__file__).parents[2] / "scripts" / "measure_complexity.py"),
            "--changed-from",
            "HEAD~1",
            "--max-complexity",
            "3",
