...
def test_pdf_history_paginates_by_entries_per_page() -> None:
    """Approval history splits into pages once it exceeds the per-page limit."""
    trip = _sample_trip_plan()
    history = [_event(i) for i in range(6)]
...
def test_pdf_rejects_non_positive_entries_per_page() -> None:
    """The generator refuses non-positive pagination sizes."""
    trip = _sample_trip_plan()
    ...
    try:
        ...
        assert "entries_per_page" in str(exc)
    else:
        raise AssertionError("Expected ValueError for entries_per_page=0")
